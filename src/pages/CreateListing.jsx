...
const CreateListing = () => {
  ...
  const onSubmit = async (e) => {
    ...
    if(geolocationEnabled) {
      const response = await fetch(`https://maps.googleapis.com/maps/api/geeocode/json?address=${address}&key=${process.env.REACT_APP_GEOCODE_API_KEY}`);
    ...
    <div className='profile'>
      ...
      <main>
        <form onSubmit={onSubmit}>
          ...
          <input type="text" id="name" value={name} onChange={onMutate} maxLength='32' minLength='10' className="formInputName" required />

          <div className="formRooms flex">
            <div>
              <label className="formLabel">Bedrooms</label>
              <input type="number" id="bedrooms" value={bedrooms} onChange={onMutate} min='1' max='50' className="formInputSmall" required />
            </div>
            <div>
              <label className="formLabel">Bathrooms</label>
              <input type="number" id="bathrooms" value={bathrooms} onChange={onMutate} min='1' max='50' className="formInputSmall" required />
            </div>
          </div>
          ...
          <div className="formButtons">
            <button className={offer ? 'formButtonActive' : 'formButton'} type='button' id='offer' value={true} onClick={onMutate} min='1' max='50'>
              Yes
            </button>
          ...
          <div className="formPriceDiv">
            <input type="number" id="regularPrice" className="formInputSmall" value={regularPrice} onChange={onMutate} min='50' max='750000000' required />
            {type === 'rent' && (
          ...
          {offer && (
            <>
              ...
              <input type="number" id="discountedPrice" className="formInputSmall" value={discountedPrice} onChange={onMutate} min='50' max='750000000' required={offer} />
            </>
          )}
